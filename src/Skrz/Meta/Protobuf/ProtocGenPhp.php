...
class ProtocGenPhp
{
	...
	private function generateEnum($className, EnumDescriptorProto $enum)
	{
		...
		if (($info = $this->getSourceCodeInfo($className)) && $info->getLeadingComments()) {
			$class->addDocument(trim($info->getLeadingComments()));
		}
	...
	private function generateMessage($className, DescriptorProto $message)
	{
		...
		foreach ((array)$message->getField() as $i => $field) {
			...
			if (($info = $this->getSourceCodeInfo($className, array_merge($this->paths[$className], [DescriptorProtoMeta::FIELD_PROTOBUF_FIELD, $i]))) &&
				...
				$property->addDocument(str_replace("*/", "* /", trim($info->getLeadingComments())));
			...
			$property->addDocument("@var {$phpType}");

			if (isset($see)) {
				$property->addDocument("@see {$see}");
				unset($see);
			}
			...
			$property
				->addDocument("")
				->addDocument(
					"@Skrz\\Meta\\Protobuf\\ProtobufField(" .
					"number={$field->getNumber()}" .
					", wireType=\"{$wireType}\"" .
					", unsigned=" . var_export(isset($unsigned) ? $unsigned : false, true) .
					", packed=" . var_export($field->getOptions() ? $field->getOptions()->getPacked() : false, true) .
					")"
				);
